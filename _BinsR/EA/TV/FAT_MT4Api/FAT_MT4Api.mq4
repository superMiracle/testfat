...
extern string ex_sServerName = "FAT_TradeView";
...
void process_reqNewOrder()
{
   ...
   int nTicket = OrderSend(sSymbol, nCmd, NormalizeDouble(dLots,1), NormalizeDouble(dCurPrice, nDigits), 0, 0, 0, NULL, MAGIC);
   if ( nTicket < 0 ) 
   {
...
void process_reqOrderClose()
{
   ...
   bool bRet = OrderClose(nTicket, NormalizeDouble(dLots,1), NormalizeDouble(dClosePrice, nDigits),3 );
   if (bRet )
   {
