...
{
    class CSiteMT5 : CSite
    {
        CMT5ApiDLL m_mt5ApiDLL = new CMT5ApiDLL();
        public override bool OnInit()
        {
            CFATLogger.output_proc("connecting to pipe : " + m_sPipe_order + "," + m_sPipe_rate);
            if ( !m_mt5ApiDLL.connectToMT4(m_sPipe_rate, m_sPipe_order) )
            {
                CFATLogger.output_proc(string.Format("site = {0} : Cannot connect to PIP, pipe name = {1}, {2}, please check MT4 and parameters of EA!",
                    m_sSiteName, m_sPipe_rate, m_sPipe_order));
                return false;
            }
            ...
            {
                m_mt5ApiDLL.mt4_registerSymbol(m_sSymbols[i]);
            }
            m_mt5ApiDLL.mt4_startSubscribe();
            return base.OnInit();
        }
        ...
        public override EERROR OnTick()
        {
            ...
            {
                if (!m_mt5ApiDLL.mt4_getRates(sSymbol, ref dAsk, ref dBid))
                {
        ...
        public override bool updateRealPositions()
        {
            ...
            List<TMT4PosItem> lstPosMT4 = m_mt5ApiDLL.mt4_getPositions();
        ...
        private bool closeOrder(string sSymbol, ETRADER_OP nCmd, ref double dLots, ref double dPrice, EORDER_TYPE nOrderType, string sLogicID, string sComment = "")
        {
            ...
            for (int i = 0; i < m_lstPos_real.Count; i++)
            {
                ...
                if (dRemainLots >= posItem.m_dLots_exc)
                {
                    ...
                    bRet = m_mt5ApiDLL.mt4_reqCloseOrder(posItem.m_nTicket, ref posItem.m_dLots_exc, ref dPrice);
                ...
                else
                {
                    ...
                    bRet = m_mt5ApiDLL.mt4_reqCloseOrder(posItem.m_nTicket, ref dRemainLots, ref dPrice);
                    dRemainLots = 0;
                }
